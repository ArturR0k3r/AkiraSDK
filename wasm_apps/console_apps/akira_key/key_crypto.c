/*
 * key_crypto.c — SHA-1, SHA-256, HMAC, PBKDF2, ChaCha20, TOTP, base32, ed25519 stub
 * SPDX-License-Identifier: Apache-2.0
 *
 * SHA-1:    FIPS 180-4  (used for TOTP per RFC 4226)
 * SHA-256:  FIPS 180-4  (used for PBKDF2, vault key derivation)
 * ChaCha20: RFC 7539
 * PBKDF2:   RFC 2898
...
static void sha1(const uint8_t *d, int l, uint8_t *out) {
    uint32_t h[5] = {0x67452301u,0xEFCDAB89u,0x98BADCFEu,0x10325476u,0xC3D2E1F0u};
    uint8_t blk[64]; int i, rem = l % 64;
    uint64_t bits = (uint64_t)l * 8;
    int pos = 0;
    while (pos + 64 <= l) { sha1_block(h, d + pos); pos += 64; }
    for (i = 0; i < rem; i++) blk[i] = d[pos + i];
    blk[rem] = 0x80;
    for (i = rem + 1; i < 64; i++) blk[i] = 0;
    if (rem < 56) {
...
static void hmac_sha1(const uint8_t *k, int kl, const uint8_t *m, int ml, uint8_t *out) {
    uint8_t ki[64], ko[64], tmp[20]; int i;
    if (kl > 64) { sha1(k, kl, tmp); kl = 20; for (i=0;i<20;i++) ki[i]=tmp[i]; }
    else { for (i = 0; i < kl; i++) ki[i] = k[i]; }
    for (i = kl; i < 64; i++) ki[i] = 0;
    for (i = 0; i < 64; i++) { ko[i] = ki[i] ^ 0x5Cu; ki[i] ^= 0x36u; }

    /* inner = SHA1(ki || m) */
    uint32_t h[5] = {0x67452301u,0xEFCDAB89u,0x98BADCFEu,0x10325476u,0xC3D2E1F0u};
    sha1_block(h, ki); /* ki is 64 bytes */
    uint8_t blk[64]; int pos = 0, rem = ml % 64;
    uint64_t bits = (uint64_t)(64 + ml) * 8;
    while (pos + 64 <= ml) { sha1_block(h, m + pos); pos += 64; }
    int r2 = ml - pos;
    for (i = 0; i < r2; i++) blk[i] = m[pos+i];
    blk[r2] = 0x80;
    for (i = r2+1; i < 64; i++) blk[i] = 0;
    if (r2 < 56) {
        for (i = 0; i < 8; i++) blk[56+i] = (uint8_t)(bits >> ((7-i)*8));
        sha1_block(h, blk);
    } else {
        sha1_block(h, blk); (void)rem;
        for (i = 0; i < 56; i++) blk[i] = 0;
        for (i = 0; i < 8; i++) blk[56+i] = (uint8_t)(bits >> ((7-i)*8));
        sha1_block(h, blk);
    }
    uint8_t inner[20];
    for (i = 0; i < 5; i++) {
        inner[i*4]   = (uint8_t)(h[i] >> 24); inner[i*4+1] = (uint8_t)(h[i] >> 16);
        inner[i*4+2] = (uint8_t)(h[i] >>  8); inner[i*4+3] = (uint8_t) h[i];
    }
    /* outer = SHA1(ko || inner) */
    uint8_t outer_in[84];
    for (i = 0; i < 64; i++) outer_in[i] = ko[i];
    for (i = 0; i < 20; i++) outer_in[64+i] = inner[i];
    sha1(outer_in, 84, out);
}
...
/* PBKDF2-SHA256, 1 block (32-byte output). No VLA — fixed 20-byte salt buffer. */
void pbkdf2_sha256(const uint8_t *pw, int pwl,
                   const uint8_t *salt, int sl,
                   int iters, uint8_t *out) {
    uint8_t u[32], f[32], s2[20]; /* salt max 16 bytes + 4 block counter */
    int i;
    if (sl > 16) sl = 16;
    for (i = 0; i < sl; i++) s2[i] = salt[i];
    s2[sl]=0; s2[sl+1]=0; s2[sl+2]=0; s2[sl+3]=1; /* block 1 */
...
static void chacha20_block(const uint32_t in[16], uint32_t out[16]) {
    ...
    for (i = 0; i < 10; i++) {
        #define QR(a,b,c,d) \
            out[a]+=out[b]; out[d]^=out[a]; out[d]=ROTL(out[d],16); \
...
void chacha20_xor(const uint8_t *key, const uint8_t *nonce, uint32_t ctr,
    ...
    while (pos < len) {
        ...
        for (i = 0; i < blk; i++) data[pos+i] ^= ((uint8_t *)ks)[i];
        pos += blk;
    }
}
...
uint32_t totp_generate(const uint8_t *secret, int slen,
    ...
    uint64_t T = unix_sec / (uint64_t)period;
    uint8_t msg[8];
    for (int i = 7; i >= 0; i--) { msg[i] = (uint8_t)(T & 0xFF); T >>= 8; }
    uint8_t mac[20];
    hmac_sha1(secret, slen, msg, 8, mac);    /* RFC 4226 §5.3 */
    int off = mac[19] & 0x0F;
...
/* ── Base32 decode ───────────────────────────────────────────────────── */
static int base32_val(unsigned char c) {
...
int base32_decode(const char *b32, uint8_t *out, int max) {
    ...
    for (int i = 0; b32[i] && b32[i] != '=' && n < max; i++) {
        int v = base32_val((unsigned char)b32[i]);
        if (v < 0) continue;          /* skip spaces, dashes, invalid */
...
void ed25519_generate_keypair(const uint8_t *seed32, uint8_t pub[32], uint8_t priv[64]) {
    /* Store seed; derive stub public key via SHA-256 of seed.
     * Real impl would do scalar multiplication on Curve25519. */

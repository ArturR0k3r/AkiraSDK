...
static void draw_status(const char *name, int ble) {
    trow_bg(0, C_HEADER);
    char buf[18]; buf[0]='[';
...
static void draw_home(void) {
    display_clear(C_BG);
    draw_status("HOME", g_key_vault.ble_enabled);
    clear_content();
...
static void draw_totp_list(void) {
    display_clear(C_BG);
    draw_status("TOTP/HOTP",g_key_vault.ble_enabled);
    clear_content();
    ...
    tput(0,ROWS-2,"  [ENTER] view code",C_DIM);
...
static void draw_totp_view(void) {
    display_clear(C_BG);
    draw_status("TOTP",g_key_vault.ble_enabled);
    clear_content();
    ...
    cbuf[digits]='\0';
    char disp[12]; int half=digits/2;
...
static void draw_fido2_list(void) {
    display_clear(C_BG);
    draw_status("FIDO2",g_key_vault.ble_enabled);
    clear_content();
...
static void draw_fido2_view(void) {
    display_clear(C_BG);
    trow_bg(0,C_DANGER);
    draw_status("FIDO2/AUTH",g_key_vault.ble_enabled);
    clear_content();
...
static void draw_pass_list(void) {
    display_clear(C_BG);
    draw_status("PASSWORDS",g_key_vault.ble_enabled);
    clear_content();
...
static void draw_ssh_view(void) {
    display_clear(C_BG);
    draw_status("SSH AGENT",g_key_vault.ble_enabled);
    clear_content();
...
static void draw_settings(void) {
    display_clear(C_BG);
    draw_status("SETTINGS",g_key_vault.ble_enabled);
    clear_content();
    ...
    for(int i=0;i<3;i++){
        ...
        if(i==0) tput(COLS-4,row,g_key_vault.ble_enabled?"ON ":"OFF",
...
void ui_init(void) {
    list_sel=0; list_scroll=0; hold_frames=0;
}
...
void ui_handle_key(int key, int long_press) {
    ...
    case SCR_TOTP_VIEW:
        ...
        if((key==KEY_A||key==KEY_RIGHT) && g_key_vault.ble_enabled){
            ...
            uint32_t c=totp_code; code_str[digs]='\0';
    ...
    case SCR_SETTINGS:
        ...
        if(key==KEY_RIGHT){
            ...
            if(list_sel==1){ hold_frames=0; go(SCR_FACTORY_RESET); }

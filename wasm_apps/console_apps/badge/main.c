                   ...
                   const char *org, const char *role)
{
    ...
    /* Handle — large font, vertically centered around y=100 */
    int handle_x = SCR_W / 2 - str_len(handle) * 11 / 2;
    display_text_large(handle_x, 82, handle, COL_ACCENT);
    ...
    if (str_len(role) > 0) {
        ...
        display_text(rx + 8, 179, role, COL_BG);
    }

    /* Very dim hint at very bottom edge */

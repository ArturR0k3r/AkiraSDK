...
/* Seed from an external value (see random_seed() below) */
...
/**
 * @brief Produce a fresh, unpredictable seed for the piece sequence.
 *
 * Primary source is the ESP32-S3 hardware RNG (crypto_random).  A frame
 * counter alone is not enough: if the title-screen wait exits immediately —
 * which it does when a button reads as stuck-pressed — the counter is the
 * same on every boot and so is the whole piece sequence.
 *
 * Falls back to uptime mixed with @p mix when the RNG is unavailable (e.g.
 * the "crypto" capability is denied), so the seed still varies per run.
 *
 * @param mix  Extra entropy to fold into the fallback (title-screen frames).
 * @return Seed value for rng_seed().
 */
static uint32_t random_seed(uint32_t mix)
{
    uint32_t s = 0;
    if (crypto_random(&s, sizeof(s)) == 0 && s != 0)
        return s;

    /* Fallback: uptime in ms at the moment of the key press, mixed with the
     * frame count via Knuth's multiplicative hash. */
...
{
    if (!force && g.next_piece == g.prev_next)
        return;
...
static void auto_drop(void)
{
    ...
    if (n > 0)
    {
        ...
        if (g.drop_delay < MIN_DROP_DELAY)
            g.drop_delay = MIN_DROP_DELAY;
    }
    spawn_piece();
}
...
static int show_pause_menu(void)
{
    ...
    while (1)
    {
        ...
        if (u && !prev_u)
        {
            cur = (cur > 0) ? cur - 1 : MENU_ITEMS - 1;
            redraw = 1;
        }
        if (d && !prev_d)
        {
            cur = (cur < MENU_ITEMS - 1) ? cur + 1 : 0;
            redraw = 1;
        }
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (g_restart_game)
        {
            /* Fresh draw from the hardware RNG on each restart, too */
            rng_seed(random_seed(++frames));
            continue;
        }

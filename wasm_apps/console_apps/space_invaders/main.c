...
{
    if (row == 0)
        return COL_ALIEN_3; /* top row = red (most points) */
    if (row <= 2)
        return COL_ALIEN_2; /* middle = yellow */
...
static int handle_input(void)
{
    ...
    if (left && ship_x > 0)
        ship_x -= SHIP_SPEED;
    if (right && ship_x < SCR_W - SHIP_W)
        ship_x += SHIP_SPEED;
    if ((a && !prev_a) || (b && !prev_a))
        fire_bullet();
    if (s && !prev_s)
    {
        prev_s = s;
...
static void update_aliens(void)
{
    ...
    int need_drop = 0;
    if (alien_dir > 0 && right_px + alien_step_x >= SCR_W - 4)
        need_drop = 1;
    if (alien_dir < 0 && left_px - alien_step_x <= 4)
        need_drop = 1;
